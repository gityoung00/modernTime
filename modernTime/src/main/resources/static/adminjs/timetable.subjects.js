  ...
  var _fn = {
	...
	_fn.loadLecture();
      if ($('body').is(':has(#subjects)')) {
        $subjects = $('#subjects');
        _set.subjectSchoolId = 7;//Number($('#userSchool').val());
        _set.subjectCampusId = 11;//Number($('#userCampus').val());
        _set.subjectColumnInfo =7;
...
//        }
    ...
    open: function () {
      ...
      $filterItems
      .on('click',subjects > div.filter >  'a.item[data-id="category"]', function () {
        _fn.showCategoryFilter();
      }).on('click',subjects > div.filter > 'a.item[data-id="category"] > span.reset', function (event) {
        _fn.resetCategoryFilter();
        event.stopPropagation();
      }).on('click',subjects > div.filter > 'a.item[data-id="keyword"]', function () {
        _fn.showKeywordFilter();
      }).on('click',subjects > div.filter > 'a.item[data-id="keyword"] > span.reset', function (event) {
        _fn.resetKeywordFilter();
        event.stopPropagation();
      }).on('click',subjects > div.filter > 'a.item[data-id="order"]', function () {
        _fn.showOrderFilter();
      }).on('click',subjects > div.filter > 'a.item[data-id="order"] > span.reset', function (event) {
