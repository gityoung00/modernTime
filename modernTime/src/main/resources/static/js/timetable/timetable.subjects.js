...
$().ready(function() {
	...
	var _fn = {
		...
		appendSubjects: function(data) {
			...
			$(data.data).each(function(idx, lecture) {
				var type = lecture.type
				var code = lecture.lecture_id
				var name = lecture.name
				var professor = lecture.teacher
				var credit = lecture.credit
				var place = lecture.place
				var score = lecture.score
				var listenStudent = lecture.listen_student;
				var maxStudent = lecture.max_student;
				
				// 강의시간
				var time = `${weekday[lecture.week1]} ${_fn.getTime(lecture.starttime1)}~${_fn.getTime(lecture.endtime1)}, ${weekday[lecture.week2]} ${_fn.getTime(lecture.starttime2)}~${_fn.getTime(lecture.endtime2)}`
				...
				for (var key in $tr.data("subject")) {
					if (key == "times") continue; 
					value = $tr.data("subject")[key]
